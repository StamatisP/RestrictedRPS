...
local function CardChoiceGUI(enabled)
	if enabled then
		...
		timeText:SetPos(timeText:GetPos(), 300 / guiScale)
		timeText:ShowCloseButton(false)
		timeText:SetDraggable(false)
		timeText:SetTitle("")
		timeText.Paint = function(self, w, h)
			draw.SimpleTextOutlined(string.ToMinutesSecondsMilliseconds(choiceTime), "InfoRUS3", 0, 0, Color(255, 170, 60), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 2, Color(0, 0, 0))
		end
